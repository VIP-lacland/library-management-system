                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <small class="text-muted">Barcode:
                                                        <?= htmlspecialchars($loan['barcode']) ?></small>
                                                </td>
                                                ...
                                                <td>
                                                    ...
                                                    <span class="badge bg-<?= $statusClass ?>">
                                                        <i
                                                            class="fas <?= $statusIcon ?> me-1"></i><?= ucfirst($loan['status']) ?>
                                                    </span>
                                                </td>
                                                <td>
                                                    <?php if ($loan['status'] == 'borrowing' || $loan['status'] == 'overdue'): ?>
                                                        <a href="admin.php?action=borrow-return&id=<?= $loan['loan_id'] ?>"
                                                            class="btn btn-sm btn-success"
                                                            onclick="return confirm('Confirm return book?')"><i
                                                                class="fas fa-undo"></i> Return</a>
                                                    <?php endif; ?>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                    ...
                    <nav aria-label="Page navigation" class="mt-4">
                        <ul class="pagination justify-content-center">
                            <li class="page-item <?= ($currentPage <= 1) ? 'disabled' : '' ?>">
                                <a class="page-link"
                                    href="admin.php?action=borrow-list&page=<?= $currentPage - 1 ?><?= !empty($keyword) ? '&keyword=' . urlencode($keyword) : '' ?>">Previous</a>
                            </li>
                            <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                                <li class="page-item <?= ($i == $currentPage) ? 'active' : '' ?>">
                                    <a class="page-link"
                                        href="admin.php?action=borrow-list&page=<?= $i ?><?= !empty($keyword) ? '&keyword=' . urlencode($keyword) : '' ?>"><?= $i ?></a>
                                </li>
                            <?php endfor; ?>
                            <li class="page-item <?= ($currentPage >= $totalPages) ? 'disabled' : '' ?>">
                                <a class="page-link"
                                    href="admin.php?action=borrow-list&page=<?= $currentPage + 1 ?><?= !empty($keyword) ? '&keyword=' . urlencode($keyword) : '' ?>">Next</a>
                            </li>
                        </ul>
                    </nav>
                <?php endif; ?>
